...
if ($porsi==0){
		...
		header("Location: ../vista/categorias/car/carro.php?cedula");

}else{$cliente=mysqli_fetch_object($response);
...
if ($respuesta) {
	...
	if ($data['restante']<0) {
		...
		header("Location: ../vista/categorias/car/carro.php?stockinsuficiente");
...
$res=mysqli_query($conex, $sql4) or die;("Algo ha ido mal en la eliminacion de los datos del carrito a la base de datos");
...
$res=mysqli_query($conex, $sql) or die;("Algo ha ido mal en la aprobación de los datos del carrito a la base de datos");      
...
$sql="UPDATE pedidos SET estatus='$estatus' WHERE id_usuario='$id_usuario'";
 ...
 $res=mysqli_query($conex, $sql) or die;("Algo ha ido mal en la eliminacion de los datos del pago ");      

header("Location: ../vista/categorias/carro/productos.php?notificacion");
